...
export default async function AuditPage() {
  ...
  return <><div className="page-head"><div><h1 className="page-title">Auditoria</h1><div className="page-sub">Histórico de acessos, exportações, reprocessamentos e alterações administrativas.</div></div><span className="admin-only-badge"><ScrollText size={15} />{records.length} eventos</span></div><section className="panel audit-panel"><div className="table-wrap"><table><thead><tr><th>Data e hora</th><th>Usuário</th><th>Evento</th><th>Área</th><th>Detalhes</th></tr></thead><tbody>{records.length ? records.map(record => <tr key={record.id}><td>{new Date(`${record.createdAt}Z`).toLocaleString("pt-BR")}</td><td><b>{record.userName || "Sistema/visitante"}</b><small>{record.userEmail || "—"}</small></td><td><span className="audit-action">{labels[record.action] || record.action}</span></td><td>{record.entity}</td><td><code className="audit-details">{record.details ? Object.entries(record.details).map(([key, value]) => `${key}: ${String(value)}`).join(" · ") : "—"}</code></td></tr>) : <tr><td colSpan={5} className="empty">Os próximos eventos aparecerão aqui.</td></tr>}</tbody></table></div></section></>;
}
