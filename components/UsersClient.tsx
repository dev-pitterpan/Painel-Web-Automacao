  ...
  return <>
    ...
    <div className="users-layout">
      <section className="panel users-list"><div className="panel-head"><div><div className="eyebrow">Acessos</div><div className="panel-title"><Users size={16} /> Usuários cadastrados</div></div><span className="metric-note">{users.length} conta(s)</span></div>
        {loading ? <div className="empty">Carregando usuários...</div> : <div className="table-wrap"><table><thead><tr><th>Usuário</th><th>E-mail</th><th>Perfil</th><th>Criado em</th></tr></thead><tbody>{users.map(user => <tr key={user.id}><td><b>{user.name}</b>{user.id === currentUserId && <small className="current-user">Conta atual</small>}</td><td>{user.email}</td><td><select className="role-select" value={user.role} disabled={user.id === currentUserId} onChange={event => changeRole(user.id, event.target.value)}><option value="user">Usuário</option><option value="admin">Administrador</option></select></td><td>{new Date(`${user.createdAt}Z`).toLocaleString("pt-BR")}</td></tr>)}</tbody></table></div>}
      </section>
